...
fn do_move(stack_list: &mut Vec<Vec<char>>, count: &str, from: &str, to: &str) {
    ...
    // INFO The only way to mutably refer to a slice of a Vector twice
    // INFO without causing an "double mutable borrow" error is by taking out
    // INFO the slice completely from teh source, modify it, and save back.
    // INFO In the line below we take out the slices completely, and at the
    // INFO end of this function, we restore it back
    ...
    let mut temp_stack: Vec<char> = Vec::<char>::new();
    for _i in 0..count {
